...
Reorganise from a single Astro app into the layout ADR-0005 settles, so the site and the playground can be
...
The repo is a pnpm workspace (assuming ADR-0005 lands on Option B): the current app lives under `apps/site`,
...
## Plan (Option B — pnpm workspaces; adjust if ADR-0005 picks Option C)

1. **Decide ADR-0005** (blocking).
...
3. **Create `packages/design-system/`** (populated by ADR-0003 Stage 1 — tokens + shared components).
...
- task-0001 (bugs, hygiene, dead code) and task-0002 (design system + per-page redesign) execute **inside**
